...
public class BatchNameIn {
  ...
  private List<NameIn> properNouns;
  ...
  * @throws IOException if the JSON Object is invalid with respect to BatchNameIn
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!BatchNameIn.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!BatchNameIn.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BatchNameIn` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("properNouns") != null && !jsonObj.get("properNouns").isJsonNull()) {
        ...
        if (jsonArrayproperNouns != null) {
          ...
          for (int i = 0; i < jsonArrayproperNouns.size(); i++) {
            NameIn.validateJsonObject(jsonArrayproperNouns.get(i).getAsJsonObject());
          };
        }
      }
  }
